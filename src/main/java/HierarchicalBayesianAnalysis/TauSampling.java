...
public class TauSampling extends MHSampling {
     ...
     */
    public double[] sampling(double prevTau, double prevTauDensity, double[] logOddRatios, double[] variances, double miu) {
        double curTau = this.randomTau();
        double curTauPosteriorDensity = this.posteriorTau(curTau, logOddRatios, variances, miu);
     ...
     * @return 后验概率近似值
     */
    private double posteriorTau(double curSamplingTau, double[] logOddRatios, double[] variances, double miu) {
        double v = 0, cumProd = 1;

        for (int i=0; i<variances.length; i++) {
            v += 1.0 / (Math.pow(curSamplingTau, 2) + variances[i]);
        ...
        return curTauPriorDensity * Math.pow(v, 0.5) * cumProd;
    }
}
